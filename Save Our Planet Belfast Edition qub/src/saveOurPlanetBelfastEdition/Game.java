...
public class Game {
			...
			while (iteration.hasNext()) {
				...
				if (userInput.continuePlaying().toLowerCase().equalsIgnoreCase("yes")) {
					...
					playerRollsDice(player, dice);
					 ...
					 // checks if player passed Go based on current position
					// and new position and adds tokens to account if they
					// passed GO
					passGoCollectMoreDoe(player, board, dice);
					...
					if (board.getBoardLocation(player.getBoardPosition()) instanceof Areas) {
						...
						if (!area.isAreaOwned()) {
							...
							if (areaOwner.getPlayerName().equalsIgnoreCase(player.getPlayerName())) {
								...
								System.out.printf("%nUh oh this area is owned by %s you must pay them a fee of %d%n",
										areaOwner.getPlayerName(), area.getPurchasePrice());

								// CHECK IF THEY PLAYER HAS ENOUGH FUNDS TO PLAY THE FEE
								// ASSUMING FOR NOW THAT FEE IS THE SAME AS PURCHASE PRICE, THIS CAN BE UPDATED
								// LATER
								if (player.getPlayerFunds() < area.getPurchasePrice()) {
									...
									// FEE IS PLACEHOLDER FOR NOW, IT CAN BE UPDATED WHEN WE CONFIRM WHAT THE ACTUAL
									// FEE SHOULD BE
					...
					// TO DO: WOULD BE NICE IF I COULD REFERENCE THE NAME OF THE PLAYER WHO HAS JUST
					// BEEN REMOVED
	...
	// purchase as area. The remaining players are offered the ability to purchase
	...
	// This method accepts a player and a board object and updates the areas within
	// the board objects owned variable to be not owned based on the areas that the
	// player owns
	...
	private static void printPlayerBoardPosition(Player player, Board board) {
		...
		System.out.printf("%n%s is now currently on square: %d '%s'.%n", player.getPlayerName().toUpperCase(),
		...
		if (player.getBoardPosition() < 11) {
			...
			if (playerPosition > 11) {
				...
				System.out.println("\r\n" + " _____                       _    _____  ____  \r\n"
						...
						+ "                                               \r\n" + "");
				System.out.println();
				System.out.printf("%nYou have passed Go and have received some tokens, you new balance is: %.2f%n",
						player.getPlayerFunds());
